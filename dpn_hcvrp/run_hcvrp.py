...
from options_hcvrp import get_options
...
def run(opts):
    # os.environ["CUDA_VISIBLE_DEVICES"] = opts.device_num
    ...
    val_dataset = problem.make_dataset(
        filename=f"hcvrp/hcvrp_{opts.graph_size}_3_seed24610.npz"
    )
    ...
    if opts.eval_only:
        ...
        for epoch in range(opts.epoch_start, opts.n_epochs):
            sampled_cost, greedy_cost = train_epoch(
                model,
                optimizer,
                lr_scheduler,
                epoch,
            ...
            if sampled_cost < min_sampled_cost:
                min_sampled_cost = sampled_cost
            if greedy_cost < min_greedy_cost:
                min_greedy_cost = greedy_cost

        print("Min Sampled Cost: ", min_sampled_cost)
        print("Min Greedy Cost: ", min_greedy_cost)
