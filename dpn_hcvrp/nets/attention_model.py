...
from nets.hcvrp_self_pa_encoder2 import GraphMAttentionEncoder
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False, subloss=False):
        ...
        agent_embeddings, depot_embedding, client_embs = self._init_embed(input)
        if self.handle_depot_separately:
            agent_embeddings, node_embeddings = self.embedder(agent_embeddings, depot_embedding, client_embs, agent_num=self.agent_num)
        else:
            node_embeddings = torch.cat((depot_embedding, client_embs), dim=-2)
            agent_embeddings, node_embeddings = self.embedder(agent_embeddings, node_embeddings, agent_num=self.agent_num)
        ...
        _log_p, pi, makespan, diff, cost_cl = self._inner(input, node_embeddings, agent_embeddings, subloss=subloss)
    ...
    def _init_embed(self, input):
        ...
        scale_factor = input["capacity"].max().item()
        # Embedding of depot
        depot_embedding = self.init_embed_depot(input['depot'][:, :self.depot_num, :])
        # Make the depot embedding the same for all agents
        agent_feat = torch.stack([input["capacity"] / scale_factor, input["speed"]], dim=-1)
        agent_embedding = self.agent_emb(agent_feat)
        ...
        node_feats = torch.cat([input["loc"], input["demand"].unsqueeze(-1) / scale_factor], dim=-1)

        client_embs = self.init_embed(node_feats)

        return positional_embedding, depot_embedding, client_embs
    ...
    def _inner(self, input, embeddings, agent_embeddings=None, subloss=False):
        ...
        makespan = state.get_final_cost().max(-1)[0]
    ...
    def _get_parallel_step_context(self, embeddings, state, agent_embeddings=None, problem_graph_projection=None):
        ...
        cur_node_embedding = embeddings.gather(dim=1, index=gathering_index)
        if agent_embeddings is None:
            cur_agent_embedding = embeddings.gather(dim=1, index=gathering_agent)
        else:
