...
from macsim.algorithms.model_args import PPOParams
from macsim.algorithms.base import ManualOptLearningAlgorithm
from macsim.algorithms.utils import RewardScaler, make_replay_buffer
...
class PPO(ManualOptLearningAlgorithm):
    ...
    def __init__(
        ...
        self.inner_epochs = model_params.inner_epochs
    ...
    def update(self, device):
        ...
        for _ in range(self.inner_epochs):
            for sub_td in self.rb:
                ...
                logprobs, value_pred, entropy, mask = self.policy.evaluate(sub_td, self.env)

                ratios = torch.exp(logprobs - previous_logp.detach())
                if ratios.dim() == 1:
    ...
    def training_step(self, batch, batch_idx):
        ...
        orig_td, _ = self.env.reset(batch)
        bs = orig_td.size(0)
        ...
        for i in range(0, bs, self.rollout_batch_size):
            ...
            while not next_td["done"].all():
                ...
                if td["action"].dim() == 2:
                    ...
                    logp = td["logprobs"].clone()
                    aug_bs, n_actions = action.shape
                    td = td.unsqueeze(1).expand(aug_bs, n_actions).contiguous()
                    td["action"] = action
                    td["logprobs"] = logp
                    td["reward"] = reward
                    td = td[action["pad_mask"]] #  & ~done]
