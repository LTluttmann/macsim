...
from macsim.utils.dataset import EnvLoader, get_file_dataloader
from macsim.utils.ops import all_gather_numeric, all_gather_w_padding
from macsim.algorithms.model_args import ModelParams, ModelWithReplayBufferParams
from macsim.algorithms.utils import NumericParameter, make_replay_buffer, get_grad_norm
from macsim.utils.utils import get_lightning_logger, generate_env_curriculum
...
from macsim.utils.scheduler import create_scheduler
from macsim.algorithms.losses import ce_loss

from .utils import GradNormAccumulator
...
class LearningAlgorithm(LightningModule):
    ...
    def init_from_checkpoint(
        ...
        env = Environment.initialize(params=env_params)
        policy = SchedulingPolicy.initialize(params=model_params.policy)
    ...
    def validation_step(self, batch: TensorDict, batch_idx: int, dataloader_idx: int = 0):
        ...
        reward = self.policy.generate(state, self.env)["reward"]
    ...
    def test_step(self, batch: TensorDict, batch_idx: int, dataloader_idx: int = 0):
        # if self.test_params.decode_type == "sampling":
        #     enable_dropout(self.policy)
        ...
        if self.current_epoch < epochs_until_uniform and len(env_sizes) > 1:
            ...
            if self.current_epoch == 0 and self.logger and isinstance(self.logger, WandbLogger):
                self.logger.experiment.log(
                    {
                        "Environment Probabilities until uniform": wandb.plot.line_series(
                            xs=list(range(epochs_until_uniform)),
                            ys=all_dists.T,
                            keys=[g.id for g in self.env.generators],
                            title="Environment Probabilities",
                            xname="Epoch"
                        )
                    },
                )   
    ...
    def load_state_dict(self, state_dict, *args, **kwargs):
        ...
        # remap old parameter names if necessary
        if "policy.decoder.pointer.action_emb.wait_emb" in state_dict:
            state_dict["policy.decoder.pointer.action_emb.skip_token"] = state_dict.pop("policy.decoder.pointer.action_emb.wait_emb")
        if "ref_policy.decoder.pointer.action_emb.wait_emb" in state_dict:
            state_dict["ref_policy.decoder.pointer.action_emb.skip_token"] = state_dict.pop("ref_policy.decoder.pointer.action_emb.wait_emb")
...
class LearningAlgorithmWithReplayBuffer(ManualOptLearningAlgorithm):
    def __init__(
        ...
        pin_memory = False # True if model_params.buffer_storage_device == "cpu" else False
        # make replay buffer
        self.rbs = {
    ...
    def test_step(self, batch: TensorDict, batch_idx: int, dataloader_idx: int = 0):
        ...
        while duration < self.test_params.active_search_params.time_budget:
            ...
            for _ in range(self.as_inner_epochs):
                ...
                # (bs)
                loss = ce_loss(logp, entropy, sub_td, mask=mask).mean()
